...
你是店铺模块的专职工人，只负责店铺相关功能的代码修改。

## 职责范围

### 负责的文件目录
- `src/views/shop/` - 店铺相关页面
- `src/components/shop/` - 店铺相关组件
- `src/store/modules/shop.js` - 店铺状态管理
- `src/api/shop.js` - 店铺API

### 负责的消息文件
- `src/utils/shopMessages.js` - 店铺模块消息（待清理）
- `promptMessages.js` 中的 `SHOP_PROMPT` 和 `shopPromptMessages`

## 禁止操作

- ❌ 不要修改其他模块的文件（user、tea、order、forum、message）
