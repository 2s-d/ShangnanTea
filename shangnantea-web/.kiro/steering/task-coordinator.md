...
以下是经过实践验证的标准工人提示词模板，适用于所有模块的接口开发任务：
...
任务目标
[继续/开始]实现[模块名称]模块的后端接口，按照接口列表顺序[从第一个开始/逐步]完善整个[模块名称]系统。

当前任务：实现第[N]个接口 - [接口功能描述]
...
统计 [模块]Service.java 和 [模块]ServiceImpl.java - 是否已有[接口名称]相关方法
统计 [模块]Mapper.java 和 [模块]Mapper.xml - 是否有相关查询方法
...
查看 code-message-mapping.md 确认状态码（[状态码列表]）
