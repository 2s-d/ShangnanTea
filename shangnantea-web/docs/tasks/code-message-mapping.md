...
#### 接口22: alipayNotify - /order/alipay/notify (4个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 5147 | 支付回调处理成功 | 支付宝异步通知处理成功 | 不显示（内部） |
| 5148 | 支付回调签名验证失败 | 支付宝签名验证失败 | 不显示（内部） |
| 5149 | 支付回调订单不存在 | 回调中的订单不存在 | 不显示（内部） |
| 5150 | 支付回调订单状态异常 | 订单状态不是待付款 | 不显示（内部） |

**说明**：
- 此接口为支付宝异步回调接口，由支付宝服务器调用
- 所有状态码仅用于内部日志记录，不会显示给用户
- 回调成功时返回字符串 "success" 给支付宝
- 回调失败时返回字符串 "failure" 给支付宝
...
| 22 | alipayNotify | POST | /order/alipay/notify | 5147 | 5148, 5149, 5150 | 支付宝异步回调 |
