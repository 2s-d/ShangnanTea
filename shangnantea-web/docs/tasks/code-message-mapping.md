...
|------|------|------|
| 200 | 操作成功 | 获取首页数据成功 |
| 6160 | 获取首页数据失败 | 获取首页数据失败 |

#### 接口106: updateHomeData (2个状态码)

| Code | 消息 | 场景 |
|------|------|------|
| 6060 | 区块内容已保存 | 更新首页数据成功 |
| 6163 | 保存失败 | 更新首页数据失败 |
...
| Code | 消息 | 场景 |
|------|------|------|
| 7011 | 所有通知已标记为已读 | 批量标记已读成功 |
| 7110 | 标记已读失败 | 批量标记已读失败 |

#### 接口151: batchDeleteNotifications (2个状态码)

| Code | 消息 | 场景 |
|------|------|------|
| 7013 | 所有通知已清空 | 批量删除通知成功 |
| 1100 | 操作失败 | 批量删除通知失败 |
...
| Code | 消息 | 场景 |
|------|------|------|
| 7003 | 图片上传成功 | 发送图片消息成功 |
| 7103 | 图片上传失败 | 发送图片消息失败 |
| 1103 | 不支持的文件类型 | 文件类型错误 |
| 1104 | 文件大小超限 | 文件过大 |
...
|------|------|------|
| 200 | 操作成功 | 获取用户主页成功 |
| 7120 | 未能获取到用户信息 | 获取用户主页失败 |
| 7121 | 用户信息不完整 | 用户信息不完整 |
