...
**本文档是状态码映射的唯一数据源，必须与代码保持完全同步！**

### 同步维护要求

1. **CODE_MAP 同步**：
   - `src/utils/apiMessages.js` 中的 `CODE_MAP` 必须包含本文档中列出的**所有状态码**
   - 状态码和消息必须与本文档中的定义完全一致
   - 数据来源：`CODE_MAP` 的数据必须从本文档中提取

2. **SILENT_CODES 同步**：
   - `src/utils/apiMessages.js` 中的 `SILENT_CODES` 列表必须包含本文档中所有标注 `[静默]` 的状态码
   - 文档中标注 `[静默]` 的状态码，代码中必须加入 `SILENT_CODES`
   - 文档中未标注 `[静默]` 的状态码，代码中不应加入 `SILENT_CODES`

3. **更新流程**：
   - 修改本文档 → 同步更新 `apiMessages.js` 中的 `CODE_MAP` 和 `SILENT_CODES`
   - 新增状态码 → 在本文档中添加 → 在 `CODE_MAP` 中添加
   - 修改静默状态码 → 在本文档中修改 `[静默]` 标注 → 在 `SILENT_CODES` 中同步

4. **验证检查**：
   - 提交代码前，必须验证文档和代码的一致性
   - 确保所有状态码都有对应的消息映射
   - 确保所有 `[静默]` 标注的状态码都在 `SILENT_CODES` 中
...
| 2105 | 登录失败，服务器返回的Token无效 | Token无效 | 显示 |
...
| 2120 | 获取用户列表失败 | 获取用户信息失败 | 显示 |

#### 接口7: updateUserInfo (2个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2010 | 个人资料更新成功 | 更新用户信息成功 | 显示 |
| 2110 | 个人资料更新失败 | 更新用户信息失败 | 显示 |

#### 接口8: uploadAvatar (4个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2012 | 头像更新成功 | 上传头像成功 | 显示 |
| 2112 | 头像更新失败 | 上传头像失败 | 显示 |
...
| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2011 | 密码修改成功，请使用新密码登录 | 修改密码成功 | 显示 |
| 2111 | 密码修改失败，请检查原密码是否正确 | 原密码错误 | 显示 |
...
| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2004 | 密码重置成功 | 重置密码成功 | 显示 |
| 2104 | 密码重置失败 | 重置密码失败 | 显示 |
...
| 4160 | 获取地址失败 | 获取地址列表失败 | 显示 |

#### 接口12: addAddress (2个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 4060 | 地址添加成功 | 添加地址成功 | 显示 |
| 4161 | 保存地址失败 | 添加地址失败 | 显示 |
...
| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 1004 | 更新成功 | 设置默认地址成功 | 显示 |
| 1100 | 操作失败 | 设置默认地址失败 | 显示 |

#### 接口16: submitShopCertification (2个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 1000 | 操作成功 | 提交商家认证成功 | 显示 |
| 1100 | 操作失败 | 提交商家认证失败 | 显示 |
...
| 1102 | 加载失败 | 获取关注列表失败 | 显示 |

#### 接口19: addFollow (2个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 5000 | 已关注店铺 | 添加关注成功 | 显示 |
| 5102 | 操作失败 | 添加关注失败 | 显示 |
...
| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 3011 | 已取消收藏 | 取消收藏茶叶成功 | [静默] |
| 6013 | 已取消收藏 | 取消收藏帖子成功 | [静默] |
| 3110 | 操作失败 | 取消收藏失败 | 显示 |
...
| 1102 | 加载失败 | 获取用户偏好设置失败 | 显示 |

#### 接口27: updateUserPreferences (2个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2013 | 偏好设置更新成功 | 更新用户偏好设置成功 | 显示 |
| 2114 | 偏好设置更新失败 | 更新用户偏好设置失败 | 显示 |
...
| 2120 | 获取用户列表失败 | 获取用户列表失败 | 显示 |
| 2124 | 您没有权限执行此操作 | 权限不足 | 显示 |

#### 接口29: createAdmin (3个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2023 | 管理员已添加 | 创建管理员成功 | 显示 |
| 2123 | 提交失败 | 创建管理员失败 | 显示 |
| 2124 | 您没有权限执行此操作 | 权限不足 | 显示 |
...
| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2020 | 用户已删除 | 删除用户成功 | 显示 |
| 2121 | 删除用户失败 | 删除用户失败 | 显示 |
| 2124 | 您没有权限执行此操作 | 权限不足 | 显示 |
...
| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 2021 | 用户状态已修改 | 切换用户状态成功 | 显示 |
| 2122 | 修改用户状态失败 | 切换用户状态失败 | 显示 |
| 2124 | 您没有权限执行此操作 | 权限不足 | 显示 |
...
| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 3001 | 茶叶详情加载成功 | 获取茶叶详情成功 | [静默] |
| 3101 | 加载茶叶详情失败 | 获取茶叶详情失败 | 显示 |

#### 接口39: updateTea (2个状态码)

| Code | 消息 | 场景 | 显示 |
|------|------|------|------|
| 3025 | 茶叶更新成功 | 更新茶叶成功 | 显示 |
| 3125 | 操作失败 | 更新茶叶失败 | 显示 |
