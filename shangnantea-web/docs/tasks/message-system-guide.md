...
`apiMessages.js` 中的 `CODE_MAP` 和 `SILENT_CODES` 必须与 `code-message-mapping.md` 文档保持**完全同步**：

1. **CODE_MAP 同步**：
   - 文档中的所有状态码和消息必须出现在 `CODE_MAP` 中
   - 文档更新时，必须同步更新 `CODE_MAP`
   - 状态码去重：同一状态码只保留一个消息（四位数状态码足够分配，不会重复）

2. **SILENT_CODES 同步**：
   - 文档中标注 `[静默]` 的状态码必须出现在 `SILENT_CODES` 中
   - 文档中未标注 `[静默]` 的状态码不应出现在 `SILENT_CODES` 中
   - 文档更新时，必须同步更新 `SILENT_CODES`

3. **数据来源**：
   - **唯一数据源**：`code-message-mapping.md` 是状态码和消息的权威来源
   - **代码实现**：`apiMessages.js` 从文档中提取并实现逻辑
   - **同步原则**：文档更新 → 代码更新，保持一致性

**同步更新原则：**
- 代码中的 `SILENT_CODES` 列表必须与文档中的 `[静默]` 标注保持一致
- 代码中的 `CODE_MAP` 必须包含文档中的所有状态码和消息
- 新增状态码时，必须同时更新文档和代码
- 移除状态码时，必须同时从文档和代码中删除
- 修改状态码消息时，必须同时更新文档和代码

**维护流程：**
1. **在 `code-message-mapping.md` 中更新**：
   - 添加/修改状态码和消息
   - 标注需要静默的状态码为 `[静默]`
2. **在 `apiMessages.js` 中同步更新**：
   - 更新 `CODE_MAP`：添加/修改状态码和消息映射
   - 更新 `SILENT_CODES`：添加/移除静默状态码
3. **验证测试**：
   - 调用 `showByCode()` 验证消息显示是否正确
   - 验证静默码不显示消息
   - 验证非静默码正常显示消息
4. **提交代码**：
   - 确保文档和代码同步提交
