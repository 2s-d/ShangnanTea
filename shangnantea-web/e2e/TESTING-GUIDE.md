...
> **⚠️ 重要：文档同步维护原则**
> 
> 本文档不是终极指南，而是一个自身也在持续优化的活文档。它的价值在于：
> - 记录我们已有的亮点与成果，最大可能地传承下去
> - 有些内容只需了解存在和大致功能，成为流程中不被遗忘的一部分
> - 有些地方则必须清晰明确，确保执行不偏离初衷
> 
> **给 AI 的关键提示**：当你在工作中发现以下情况时，**必须主动更新本文档**：
> - ✅ 对测试工具（error-collector.js、auth-helper.js 等）进行了重大优化
> - ✅ 改变了测试脚本的标准模板或编写规范
> - ✅ 测试执行流程发生了重要变化
> - ✅ 发现文档内容与实际代码不符
> - ✅ 新增了重要的测试方法或最佳实践
> 
> 这样才能确保后续丢失上下文的 AI 或新加入的开发者，能够获得准确的指导，不会重复造轮子或偏离既定方向。
...
本文档是 E2E 测试体系的核心指导文档，与以下文档形成关联关系：

**上级文档（战略层）**：
- 无 - 本文档是测试体系的顶层指导

**平级文档（同级参考）**：
- `README.md` - 测试体系总览，提供快速入门和目录导航
- `../openapi_new.yaml` - API 接口定义，测试的数据源

**下级文档（实施层）**：
- `helpers/error-collector.js` - 错误收集工具实现
- `helpers/auth-helper.js` - 认证辅助工具实现
- `helpers/api-helper.js` - API 辅助工具实现
- `api-tests/**/*.spec.js` - 具体测试脚本实现
- `fixtures/test-data.json` - 测试数据定义

**关联文档（项目级）**：
- `../../src/utils/promptMessages.js` - 前端提示消息定义
- `../../docs/tasks/code-message-mapping.md` - 错误码映射文档

### 同步修改的必要性

**为什么必须同步修改？**

1. **避免误导**：陈旧的文档内容会误导后续开发，导致项目跑偏
2. **保持一致**：文档与代码不一致会造成混乱，降低开发效率
3. **传承知识**：及时更新才能真正传承亮点与成果
4. **减少重构**：持续小步更新比积累后大规模重构更高效

**什么时候需要同步修改？**

| 变更类型 | 需要同步的文档 | 优先级 |
|---------|--------------|--------|
| 修改测试工具函数（helpers/*.js） | 本文档 + README.md | 🔴 高 |
| 修改测试脚本模板 | 本文档 | 🔴 高 |
| 新增测试场景或最佳实践 | 本文档 | 🟡 中 |
| 修改测试执行流程 | 本文档 + README.md | 🔴 高 |
| 修改 API 接口定义 | openapi_new.yaml + 相关测试脚本 | 🔴 高 |
| 修改错误码或提示消息 | promptMessages.js + code-message-mapping.md | 🟡 中 |
| 优化测试数据结构 | fixtures/test-data.json + 相关测试脚本 | 🟡 中 |

**同步修改的检查清单**：

当你修改了测试相关代码后，请检查：

- [ ] 本文档（TESTING-GUIDE.md）是否需要更新？
- [ ] README.md 是否需要同步更新？
- [ ] 测试脚本模板是否需要调整？
- [ ] 相关的测试脚本是否需要批量更新？
- [ ] 是否需要更新"最后更新"时间戳？
...
如果你是 AI 助手，在完成以下任务后，**必须主动检查并更新相关文档**：

1. ✅ 优化了 `helpers/` 下的任何工具函数
2. ✅ 修改了测试脚本的标准写法
3. ✅ 改进了测试执行流程或启动脚本
4. ✅ 发现文档描述与实际代码不符
5. ✅ 新增了值得推广的测试方法

**不要等用户提醒，主动维护文档的一致性。陈旧的文档比没有文档更危险。**
