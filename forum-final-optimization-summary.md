...
| 方法 | 优化前SQL次数 | 优化后SQL次数 | 提升 |
|------|--------------|--------------|------|
| getForumPosts（100个帖子） | 201次 | 3次 | 98.5% ↓ |
| getPendingPosts（20个帖子） | 41次 | 3次 | 92.7% ↓ |
| getPostReplies（20个回复） | 41-61次* | 2-3次 | 93-95% ↓ |

*注：getPostReplies的查询次数取决于回复中有多少个toUserId（回复其他用户）

### 整体性能提升

| 指标 | 优化前 | 优化后 | 提升 |
|------|--------|--------|------|
| 平均SQL查询次数 | ~100次/请求 | ~3次/请求 | 97% ↓ |
| 数据库连接次数 | ~100次/请求 | ~3次/请求 | 97% ↓ |
| 响应时间（估算） | ~1000-2000ms | ~30-50ms | 95-97% ↓ |
| 数据库负载 | 高 | 低 | 显著降低 |
