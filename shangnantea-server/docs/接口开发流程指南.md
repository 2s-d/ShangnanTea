...
- **先检查Controller**：确认Controller中是否已有接口定义，了解方法签名
- **再定义Service**：根据Controller的方法签名定义Service接口
- **最后实现逻辑**：在ServiceImpl中实现具体的业务逻辑
- **验证一致性**：确保Controller、Service接口、Service实现三者的方法签名完全一致
...
**A:** 需要登录权限的接口应该在Controller添加 `@RequiresLogin` 注解，在Service层使用 `UserContext.getCurrentUserId()` 获取当前用户ID进行业务验证。
...
### Q8: Controller和Service方法签名不匹配怎么办？

**A:** 在实现接口时，经常会发现Controller中已有的方法签名与预期的Service方法不匹配。正确的做法是：
1. 以Controller的方法签名为准（因为它定义了对外的API契约）
2. 修改Service接口和实现以匹配Controller的参数
3. 不要随意修改Controller的方法签名，除非确认前端也需要同步修改

**实际案例**：
- `uploadShopLogo(String shopId, MultipartFile file)` - Controller有shopId参数
- `uploadTeaImages(String teaId, MultipartFile[] files)` - Controller支持批量上传
- `uploadBanner(MultipartFile file, String title, String subtitle, String linkUrl)` - Controller有额外的业务参数
...
**文档版本：** v1.3  
**最后更新：** 2026-01-21  
**基于功能：** 用户登录、用户注册、用户头像上传、图片消息发送、9个图片上传接口（补充实际开发经验）
