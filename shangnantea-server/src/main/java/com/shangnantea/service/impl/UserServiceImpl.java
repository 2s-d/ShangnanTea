...
public class UserServiceImpl implements UserService {
    ...
    // 登录会话(JTI)的实时推送改由 Redis KeyEvent 监听统一处理（避免散落在业务点）
     ...
     */
    @Override
    public Result<Boolean> forceLogout(String userId) {
        try {
            if (userId == null || userId.trim().isEmpty()) {
