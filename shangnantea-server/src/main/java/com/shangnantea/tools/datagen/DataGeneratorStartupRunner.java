...
public class DataGeneratorStartupRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        System.out.println("======================================");
        System.out.println("当前服务已启动，可选择是否立即运行交互式数据生成工具。");
        System.out.println("注意：仅建议在本地/开发环境使用，不要在生产环境生成测试数据。");
        System.out.print("是否现在运行数据生成 TUI？输入 Y 确认，其他任意键跳过：");

        BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
        String line = in.readLine();
        if (line != null && "Y".equalsIgnoreCase(line.trim())) {
            System.out.println("[DataGen-Startup] 开始运行交互式数据生成工具...");
            try {
                DataGeneratorTui.main(new String[0]);
            } catch (Exception e) {
                System.err.println("[DataGen-Startup] 数据生成工具执行失败: " + e.getMessage());
                e.printStackTrace(System.err);
            }
        } else {
            System.out.println("[DataGen-Startup] 已跳过数据生成。");
        }
    }
}
