...
**理解A：第三方平台的余额（如支付宝余额、微信零钱）**
- 这种情况下，用户选择"支付宝支付"或"微信支付"
- 第三方平台会自动处理余额不足的问题
- 我们的系统不需要关心用户在第三方平台的余额
- **这不需要我们实现余额验证**

**理解B：系统内部的余额（如会员余额、充值余额）** ⭐ 当前代码指的是这个
- 这是我们系统内部的余额系统
- 需要我们自己维护用户余额表
- 需要我们自己实现余额充值、扣减、查询逻辑
- **第三方支付接口无法处理这个**

**当前代码中的"balance"支付方式，应该是指理解B（系统内部余额）**
...
**方案2：如果项目需要支持系统内部余额支付**
- 需要创建用户余额表（user_balance或在user表中添加balance字段）
- 创建UserBalanceMapper
- 实现余额查询和扣减逻辑
- 实现余额充值功能（可以通过第三方支付充值）
- 实现余额支付的事务处理

**方案3：集成第三方支付后的推荐方案**
- 移除"balance"支付方式
- 只支持第三方支付方式：alipay（支付宝）、wechat（微信）
- 用户在第三方平台选择使用余额或银行卡，由第三方平台处理

**当前建议**：采用方案3，移除balance支付方式，只支持第三方支付
...
**完整的退款流程**：
```
1. 用户申请退款（refundOrder方法）
   ↓
2. 商家审核（processRefund方法 - 当前位置）
   ↓
3. 如果同意，调用第三方支付平台的退款接口 ⭐ 需要我们主动调用
   ↓
4. 第三方平台处理退款（异步）
   ↓
5. 第三方平台通过回调通知我们退款结果
...
if (rows > 0) {
    // 调用第三方支付平台的退款接口
    try {
        // 1. 根据订单的支付方式选择对应的支付平台
        if ("alipay".equals(order.getPaymentMethod())) {
            // 调用支付宝退款接口
            alipayService.refund(order.getId(), order.getTotalAmount());
        } else if ("wechat".equals(order.getPaymentMethod())) {
            // 调用微信退款接口
            wechatPayService.refund(order.getId(), order.getTotalAmount());
        }
        
        // 2. 退款请求已提交，等待第三方平台处理
        // 注意：退款是异步的，需要通过回调接口接收退款结果
...
public void handleRefundCallback(@RequestBody Map<String, Object> callbackData) {
    // 1. 验证回调签名
    // 2. 解析回调数据
    // 3. 更新订单状态为"已退款"
    // 4. 记录退款时间
...
- 在processRefund方法中调用退款接口
- 实现退款回调接口处理退款结果
- 退款成功后更新订单状态为"已退款"

**当前建议**：如果要集成第三方支付，必须采用方案2
